...
which we can think of as generated by an element \\( 1\_{\lambda} \\) satisfying \\( E1\_{\lambda} = 0 \\) and \\( H1\_{\lambda} = \lambda 1\_{\lambda} \\).
...
The image of \\(z \in Z(U\mathfrak{sl}\_{2}) \\) in \\(\mathbb{C}[\mathfrak{h}] = \mathbb{C}[\lambda]\\) is the function sending \\(\lambda\\) to the scalar \\( z|\_{M\_{\lambda}} \\).
