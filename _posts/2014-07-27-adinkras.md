...
> How many edge 2-colorings \(\phi : E(Q_n) to {0,1}\) exist such that, for each 4-cycle \(C\) of \(Q_n\), the sum of the edge colors is odd: \(\sum_{e \in E(C)} \phi(e) = 1\) mod \(2\) ?

Note that we bastardize the term “edge n-colorings” \(\equiv\) assignment of \(x \in {0,1,..,n-1}\) to every edge.

#### Breaking Down The Problem: The Overture

Let \(e_n\) denote the number of edge colorings of \(Q_n\) for which the sum of the edge colors is even, and let \(o_n\) denote the number of edge colorings for which the sum is odd. As \(Q_0\) contains a single vertex and no edges, there are no edge colorings with either property, so we set \(e_0 = o_0 = 1\).

Now \(Q_1\) contains two vertices with a single edge between them, so \(e_1 = o_1 = 1\). The slightly more interesting graph \(Q_2\) corresponds to a square, which can have its edges 2-colored in \(2^4\) ways. Half of which will correspond to an even sum, the other half to an odd, so \(e_2 = o_2 = 8\). The graph \(Q_3\) can be 2-colored in \(2^{12}\) ways, half of which must be even and the other half odd, so \(e_3 = o_3 = 2^{11}\).
...
[RinsProblem](/images/wp-content/uploads/2014/07/RinsProblem.pdf)

{% include pdf.html src="/images/wp-content/uploads/2014/07/RinsProblem.pdf" title="Rin's Problem" %}
...
<p class="repo-link"><span class="repo-label">Code</span> <a href="https://github.com/catherineray/graphical-supersymmetry-algebras">graphical-supersymmetry-algebras</a></p>
